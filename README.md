...
> Infrastructure-As-Code - Open source tool by AetherCodeHQ

`cli` `devops` `golang` `iac` `infrastructure`

## What is it?

**Infrastructure As Code** is a devops tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Infrastructure-As-Code --path ./target

# With options
./Infrastructure-As-Code --path ./target --format json --output report.json

# Verbose mode
./Infrastructure-As-Code --path ./target --verbose
```
...
### Basic scan
```bash
./Infrastructure-As-Code --path ./my-project
```

### JSON report
```bash
./Infrastructure-As-Code --path ./my-project --format json --output report.json
```
...
- name: Run Infrastructure As Code
  run: ./Infrastructure-As-Code --path . --format json --output report.json
```
